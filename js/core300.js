/* js/core300.js - V300.107 Final Core (Fix Rewards) */
...
const act = {
    ...
    showSysModal: (type, msg, val, callback) => { 
        ...
        const btnCancel = document.getElementById('sys-cancel'); 
        m.style.display = 'flex'; m.classList.add('active'); txt.innerText = msg; 
        ...
        if (type === 'prompt') { inp.style.display = 'block'; inp.value = val || ''; inp.focus(); } else { inp.style.display = 'none'; } 
        if (type === 'alert') { newCancel.style.display = 'none'; newOk.onclick = () => { m.style.display = 'none'; if(callback) callback(); }; } 
        else { newCancel.style.display = 'block'; newCancel.onclick = () => { m.style.display = 'none'; }; newOk.onclick = () => { const res = (type === 'prompt') ? inp.value : true; m.style.display = 'none'; if(callback) callback(res); }; } 
    },
    ...
    clearInputs: (parentId) => {
        ...
        const diffSlider = parent.querySelector('.slider'); if(diffSlider) { diffSlider.value = 2; act.updateDiffLabel(2); }
    },
    ...
    navigate: (p) => { 
        ...
        const pg = document.getElementById('page-' + targetPage); if(pg) pg.classList.add('active'); 
        ...
        if(targetPage === 'main' || targetPage === 'stats' || targetPage === 'task' || targetPage === 'shop') { if(window.view && view.renderHUD) view.renderHUD(); }
        
        const fab = document.getElementById('global-fab');
        if(fab) { fab.style.display = (targetPage === 'task') ? 'flex' : 'none'; }
        
        if(targetPage === 'stats' && window.view && view.renderStats) { setTimeout(() => view.renderStats(), 50); }
    },
    ...
    checkDaily: () => {
        ...
        if (GlobalState.lastLoginDate !== today) {
            if (GlobalState.lastLoginDate) {
                const last = new Date(GlobalState.lastLoginDate); const curr = new Date(today);
                const diffDays = Math.ceil(Math.abs(curr - last) / (1000 * 60 * 60 * 24)); 
                if (diffDays === 1) GlobalState.loginStreak = (GlobalState.loginStreak || 0) + 1; else GlobalState.loginStreak = 1; 
            ...
            GlobalState.tasks.forEach(t => {
                ...
                if (isDaily) {
                    ...
                    t.done = false; t.curr = 0; t.lastReward = null; 
                }
            });
            if (GlobalState.settings.strictMode) {
                let rustedCount = 0;
                GlobalState.skills.forEach(s => {
                    if(s.lastUsed) { const diff = Math.ceil(Math.abs(new Date() - new Date(s.lastUsed)) / (1000 * 60 * 60 * 24)); if(diff > 3) { s.isRusted = true; s.exp = Math.max(0, s.exp - 10); rustedCount++; } }
                });
    ...
    updateLoginAchievement: () => {
        const achId = 'sys_login_streak'; let ach = GlobalState.achievements.find(a => a.id === achId);
        if (!ach) { ach = { id: achId, title: '🔥 每日簽到', desc: '', type: 'manual', targetVal: 7, reward: { freeGem: 5 }, done: false, isSystem: true }; GlobalState.achievements.unshift(ach); }
    ...
    handleFab: () => { 
        if (TempState.taskTab === 'ach') { 
            if(window.act.openCreateAch) window.act.openCreateAch(); else act.openModal('create-ach'); 
            return; 
        } 
        ...
        // 呼叫 task300.js 的初始化
        if (window.act.initCreateModal) {
            window.act.initCreateModal();
        } 
        act.openModal('create'); 
    },
    
    refreshSkillSelect: () => { const sel = document.getElementById('nt-skill-select'); if(!sel) return; sel.innerHTML = '<option value="" disabled selected>選擇技能標籤...</option>'; GlobalState.skills.forEach(s => { const attrName = GlobalState.attrs[s.parent] ? GlobalState.attrs[s.parent].name : '未知'; sel.innerHTML += `<option value="${s.name}">${s.name} (${attrName})</option>`; }); },
    ...
    toggleTask: (id) => {
        ...
        if (!t.done) {
            ...
            if(t.attrs && t.attrs.length > 0) {
                // 每個綁定的屬性獲得經驗 (平分或全拿? 這裡設定為全拿)
                t.attrs.forEach(attrName => {
                    const key = Object.keys(GlobalState.attrs).find(k => GlobalState.attrs[k].name === attrName);
                    if(key) GlobalState.attrs[key].exp += reward.exp;
                });
            }
            ...
            if (t.lastReward) {
                ...
                if(t.attrs && t.attrs.length > 0) {
                    t.attrs.forEach(attrName => {
                        const key = Object.keys(GlobalState.attrs).find(k => GlobalState.attrs[k].name === attrName);
                        if(key) GlobalState.attrs[key].exp = Math.max(0, GlobalState.attrs[key].exp - t.lastReward.exp);
                    });
                }
                t.lastReward = null;
                // act.alert("已取消，獎勵回收。");
    ...
    addSubtask: () => {
        ...
        row.innerHTML = `<input class="inp" placeholder="步驟..." style="flex:1;"><button class="btn-del" style="width:40px;" onclick="this.parentElement.remove()">✕</button>`; 
        div.appendChild(row);
    },
    toggleSubtask: (tid, sIdx) => {
        ...
        if(t && t.subs[sIdx]) { 
            ...
            const allDone = t.subs.every(s => s.done); const anyDone = t.subs.some(s => s.done);
            if (!t.done) {
                if (t.subRule === 'all' && allDone) act.toggleTask(tid);
                else if (t.subRule === 'any' && anyDone) act.toggleTask(tid);
            }
            act.save(); view.renderTasks(); 
        }
    }
};
