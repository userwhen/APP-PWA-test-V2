/* js/modules/shop105.js - V300.42 Complete & Fixed */
...
const SHOP_CONFIG = { 
    INFINITE_QTY: 999, 
    MAX_INPUT: 9999, 
    PERM_TYPE: { DAILY: 'daily', ONCE: 'once' }, 
    CATEGORY: { CALORIE: '熱量', MONEY: '金錢', TIME: '時間', OTHER: '其他' } 
};
...
Object.assign(window.act, {
    ...
    removeFromBag: (itemName, quantity) => { 
        let removed = 0; 
        const newBag = []; 
        let toRemove = quantity; 
        for (let item of GlobalState.bag) { 
            if (item.name === itemName && toRemove > 0) { 
                toRemove--; 
                removed++; 
            } else { 
                newBag.push(item); 
        ...
        act.prompt(`購買 [${item.name}]\n單價: $${item.price}\n庫存: ${shopItem.qty}\n\n請輸入購買數量:`, "1", (input) => {
            ...
            act.confirm(`確定花費 $${totalCost} 購買 ${qty} 個 [${item.name}] ?`, (yes) => {
                if (yes) {
                    ...
                    if (shopItem.perm !== SHOP_CONFIG.PERM_TYPE.DAILY || shopItem.qty < SHOP_CONFIG.INFINITE_QTY) { 
                        shopItem.qty -= qty; 
                    }
                    act.save(); 
                    act.alert("購買成功！");
    ...
    useItem: (discard) => {
        ...
        let msg = "";
        if (!discard) { 
            const effectText = act.getItemEffectText(def, qty); 
            msg = `確定要使用 ${qty} 個「${name}」？\n\n${effectText}`; 
        } else { 
            msg = `確定要丟棄 ${qty} 個「${name}」？(無法復原)`; 
        }

        act.confirm(msg, (yes) => {
            if (yes) {
                const removedCount = act.removeFromBag(name, qty);
                if (removedCount > 0 && !discard) { 
                    act.applyItemEffect(def, removedCount); 
                }
                act.save();
                
                // 更新介面
                if(window.view) {
                    if(view.renderHUD) view.renderHUD();
                    if(view.renderBag) view.renderBag(); // 假設 view 有這個函數
                }
                ...
                if (newOwned <= 0) { 
                    ...
                    if(bagGrid && window.view && view.renderBag) view.renderBag();
                }
            }
        });
    },
    ...
    openUpload: () => { 
        ...
        if(catSel) {
            catSel.value = SHOP_CONFIG.CATEGORY.CALORIE; // 預設熱量
    ...
    uploadCategoryChange: () => { 
        ...
        if (c === SHOP_CONFIG.CATEGORY.CALORIE) { 
            dyn.innerHTML = `<div class="row"><input id="up-cal" type="tel" class="inp flex-1" placeholder="卡路里 (4位數)" maxlength="4" oninput="act.validateNumber(this, 9999)"></div>`; 
    ...
    editShopItem: (id) => {
        ...
        if (item) {
            ...
            setTimeout(() => {
                ...
                if (item.category === SHOP_CONFIG.CATEGORY.TIME && document.getElementById('up-time-h')) { 
                    const [h, m] = item.val.split(':'); 
                    document.getElementById('up-time-h').value = h; 
                    document.getElementById('up-time-m').value = m; 
                }
            }, 50);
